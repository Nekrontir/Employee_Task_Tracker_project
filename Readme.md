...
Проект разворачивается с использованием Docker Compose и состоит из трёх сервисов: PostgreSQL для хранения данных, Django-приложения, запускаемого через Gunicorn, и Nginx, который выполняет роль обратного прокси и отдаёт статические файлы. Для автоматизации проверки и развёртывания настроен CI/CD-процесс на GitHub Actions: при каждом push и pull request в ветку main выполняются проверка кода, запуск тестов и автоматический деплой на удалённый сервер по SSH после успешного прохождения всех этапов.

 Запуск Docker контейнеров осуществляется при помощи команды:
