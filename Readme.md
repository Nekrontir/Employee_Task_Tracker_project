...
- HTML-интерфейс использует стандартную сессионную авторизацию Django;
...
Проект разворачивается с использованием Docker Compose и состоит из трёх сервисов: PostgreSQL, Django-приложения, запускаемого через Gunicorn, и Nginx в роли обратного прокси. Для автоматизации проверки и развёртывания настроен CI/CD-процесс на GitHub Actions: при каждом push и pull request в ветку main выполняются линтинг, тесты и автоматический деплой на удалённый сервер по SSH после успешного прохождения всех этапов.
Для создания суперпользователя в контейнере docker необходимо использовать команду: 
```bash
